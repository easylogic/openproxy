...
Session.prototype.change = function(obj) {
	...
	if (obj.file) {
		...
		if (obj.type == 'pattern') {
			...
			var subpath = session.$("path");
		}
		
		this.file(obj.dir + "/" + subpath);	

	} else if (obj.status) {
		var arr = obj.status.split("@");
...
Session.prototype.exec_connect = function() {
	  ...
	  serverSocket.setMaxListeners(0);
	  
	  serverSocket.on('error', function(e) {
		switch(e.message) {
			case "getaddrinfo ENOENT":
		    self.res.write("HTTP/1.1 400 Gateway Timeout\r\nProxy-agent:OpenProxy\r\n\r\n");
		    self.res.end();
			break;
		}
	  })
