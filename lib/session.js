...
Session.prototype.change = function(obj) {
	...
	if (obj.target.indexOf("file://") > -1) {
		...
		const targetUrls = obj.target.split(':');

		var path = this.url().replace(obj.source, targetUrls[0]);
		...
		// 커스텀 포트 지원
		if (targetUrls[1]) 	this.$('port', +targetUrls[1]);
...
Session.prototype.runRouting = function() {

	// 여긴 다시 구현해야할 듯
	// routing table 은 여기다 하면 안될 듯 하다.
	// proxy 는 순수 session 관리에만 집중하자. 


	// check pattern 
	if (this.proxy.patterns && this.proxy.patterns.length) {
		var self_url = this.url();
		for(var i = 0, len = this.proxy.patterns.length; i < len; i++) {
			var pattern = this.proxy.patterns[i];
	...
	// check domain
	if (this.proxy.domains) {
		var domain = this.proxy.domains[this.$("hostname")];
